...
def main():
    ...
    for r in m:
        ...
        sheet.cell(row=i, column=1).alignment = openpyxl.styles.Alignment(horizontal='center', vertical='center', wrap_text=True)
        sheet.cell(row=i, column=3).value = r.info
        sheet.cell(row=i, column=3).alignment = openpyxl.styles.Alignment(horizontal='center', vertical='center', wrap_text=True)
        sheet.cell(row=i, column=5).value = r.review.encode('utf-8')
        sheet.cell(row=i, column=5).alignment = openpyxl.styles.Alignment(horizontal='center', vertical='center', wrap_text=True)
...
def prepareData(reviews_df):
    ...
    reviews_df = reviews_df.assign(cleaned = reviews_df['reviewText'].apply(lambda text: ' '.join([stemmer.stem(w) for w in re.sub('[^a-z]+|(quot)+', ' ', text.lower()).split() if w not in stop_words])))
